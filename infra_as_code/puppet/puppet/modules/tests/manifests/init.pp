class tests::install {
  ...
  exec { 'create-test-database':
    unless  => '/usr/bin/mysql -u root -ppass test',
    command => '/usr/bin/mysql -u root -ppass --execute=\'create database test;\'',
  }

  exec { 'create-test-user':
    unless  => '/usr/bin/mysql -u test -ptest test',
    command => '/usr/bin/mysql -u root -ppass --execute="CREATE USER \'test\'@\'localhost\' IDENTIFIED BY \'test\'; GRANT ALL PRIVILEGES ON test.* TO \'test\'@\'localhost\'"',
  }
